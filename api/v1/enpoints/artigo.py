...
from fastapi import (
    APIRouter, status, Depends, HTTPException, Response)
...
async def post_artigo(artigo:ArtigoSchema, 
                      usuario_logado:UsuarioModel=Depends(get_current_user), # usuario logado pelo token
...
@router.delete('/{artigo_id}', response_model=ArtigoSchema, status_code=status.HTTP_202_ACCEPTED)
async def get_artigo(artigo_id:int,
                     db:AsyncSession=Depends(get_session),
                     usuario_logado:UsuarioModel=Depends(get_current_user)):
    async with db as session:
        # so pode atulizar o artigo quem criou
        query = select(ArtigoModel).where(ArtigoModel.id == artigo_id).where(
